...
async fn bounded_channel_drop_newest_example() -> Result<(), Box<dyn std::error::Error>> {
    let (sender, mut receiver) = BufferedAsyncSender::new_bounded(3, false);
    ...
    println!("Dropped events count: {}", sender.dropped_events_count());
    ...
    println!("Total received: {}, Total dropped: {}", received_count, sender.dropped_events_count());

    Ok(())
}

async fn bounded_channel_drop_oldest_example() -> Result<(), Box<dyn std::error::Error>> {
    let (sender, mut receiver) = BufferedAsyncSender::new_bounded(3, true);
    ...
    for i in 0..8 {
        ...
        sender.send_event(event).await;
        println!("Sent event {} (dropped count: {})", i + 1, sender.dropped_events_count());

        // Small delay to allow some processing
        sleep(Duration::from_millis(10)).await;
    }

    println!("Final dropped events count: {}", sender.dropped_events_count());
...
async fn unbounded_channel_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..100 {
        ...
        if i % 20 == 0 {
            println!("Sent {} events (dropped count: {})", i + 1, sender.dropped_events_count());
        }
    }
...
async fn renderer_manager_integration_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(buffered_sender) = manager.get_buffered_sender() {
        println!("Dropped events from manager: {}", buffered_sender.dropped_events_count());
    }
...
mod integration_tests {
    ...
    async fn test_send_event_channel_overflow() {
        let (sender, _receiver) = BufferedAsyncSender::new_bounded(2, false);
...
mod benchmarks {
    ...
    async fn benchmark_bounded_channel_throughput() {
        let (sender, mut receiver) = BufferedAsyncSender::new_bounded(1000, true);
        ...
        println!("  Events dropped: {}", sender.dropped_events_count());
